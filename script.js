document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addEventListener('scroll', function() {
    ...
    if (currentScroll > 100) {
      navbar.style.boxShadow = '0 4px 20px rgba(42, 42, 42, 0.15)';
    } else {
      navbar.style.boxShadow = '0 2px 12px rgba(42, 42, 42, 0.1)';
    }

    lastScroll = currentScroll;
  });

  // Form submission handler (placeholder - will be replaced with actual backend)
  ...
  // Testimonials carousel scroll behavior (RTL)
  const carousel = document.querySelector('.testimonials-carousel');
  if (carousel) {
    // Set initial scroll position to rightmost (start in RTL)
  ...
  const animatedElements = document.querySelectorAll('.feature-card, .program-card, .principle-card, .value-card, .testimonial-card, .gallery-item');
  ...
  // Details/summary (FAQ) accessibility
  const detailsElements = document.querySelectorAll('details');
  detailsElements.forEach(details => {
    const summary = details.querySelector('summary');
    if (summary) {
      summary.addEventListener('click', function(e) {
        // Smooth scroll to opened details
        setTimeout(() => {
          if (details.open) {
  ...
  console.log('אילוף מלטף - Website initialized successfully');
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אילוף מלטף משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
        ...
        function initContactForm() {
            ...
            try {
                ...
                const response = await fetch(endpoint, {
                    ...
                    body: JSON.stringify({
                        websiteId: 'd6e60221-6097-4346-918b-5c88cf80ae76',
                        name: data.name,
                        email: data.email,
